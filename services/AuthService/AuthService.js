...
import InvalidCredentialException from "../Models/ExceptionModels/InvalidCrednetialException";
import UserNotFoundException from "../Models/ExceptionModels/UserNotFoundException.js";
import InvalidPayloadException from "../Models/ExceptionModels/InvalidPayloadException.js";
...
export async function SignInFunction(email, password) {
  ...
  if (resp.status !== 200) {
    ...
    Log(
      `Response Status Code is not 200. It's Body is ${resp.body.toString()}`
    );
    switch (resp.status) {
      case 403:
      ...
      case 400:
        throw new InvalidPayloadException(
          `Invalid Payload. The payload ${payload} is invalid`
        );
      default:
